...
//  AnalyticsTests
//
//  Created by John Wolfe on 5/15/19.
//  Copyright © 2019 Segment. All rights reserved.
//

#import <XCTest/XCTest.h>
#import "../Analytics/Classes/Internal/NSData+SEGGZIP.h"
#import "Utils/NSData+OldSEGGZIP.h"
...
static NSData *seggziptests_createRandomNSData()
{
...
@interface SEGGZIPTests : XCTestCase

@end

@implementation SEGGZIPTests
...
- (void)testOutputEqualsInput
{
  ...
  //compress
  NSData *compressedData = [inputData seg_gzippedData];

  //decode
  NSData *outputData = [compressedData seg_gunzippedData];
...
- (void)testUnzipNonZippedData
{
  ...
  //decode
  NSData *outputData = [inputData seg_gunzippedData];
  NSString *outputString = [[NSString alloc] initWithData:outputData encoding:NSUTF8StringEncoding];
  XCTAssertEqualObjects(outputString, inputString);
}
...
- (void)testRezipZippedData
{
  ...
  //compress again
  NSData *outputData = [compressedData seg_gzippedData];
  XCTAssertEqualObjects(compressedData, outputData);
}

- (void)testZeroLengthInput
{
  NSData *data = [[NSData data] seg_gzippedData];
  XCTAssertEqual(data.length, 0);

  data = [[NSData data] seg_gunzippedData];
  XCTAssertEqual(data.length, 0);
}

- (void)testCompressionPerformance
{
  NSData *inputData = seggziptests_createRandomNSData();
  [self measureBlock:^{
    __unused NSData *compressedData = [inputData seg_gzippedData];
  }];
}
...
- (void)testDecompressionPerformance
{
  ...
  NSData *compressedData = [inputData seg_gzippedData];
  [self measureBlock:^{
    __unused NSData *outputData = [compressedData seg_gunzippedData];
  }];
}
...
- (void)testEncodeWriteFileDecodeInShell
{
  ...
  NSData *oldseggzippedData = [inputData old_seg_gzippedData];
  NSData *seggzippedData    = [inputData seg_gzippedData];
  XCTAssertEqualObjects(oldseggzippedData, seggzippedData);

  // compare gunzipped SEGGZIPPED data to original data
  NSData *expandedSEGGZippedData = [oldseggzippedData seg_gunzippedData];
